...
// only maps steps to pixels. The viewBox grows vertically to fit whatever
// ledger-line range the run needs (octave shifts can push it well off the
// staff). See docs/decisions/0001-notation-rendering.md.
...
// Minimal line-art clefs, drawn as strokes to match the app's aesthetic, in a
// frame where the staff's top line sits at y=10 (they get translated to the
// live frame at render time). Treble: bowl around the G line (y 34), spine,
// top curl, tail hook.
const TREBLE_PATH =
...
const CLEF_OVERHANG = 3

function ClefGlyph({ clef, y }: { clef: Clef; y: (step: number) => number }) {
...
export function ScaleStaff({
  ...
  // fit the staff, the clef's overhang, and every note (+1 step so
  // accidental glyphs and note-head strokes aren't shaved at the edges)
  const maxStep = Math.max(8 + CLEF_OVERHANG, Math.max(...steps) + 2)
  const minStep = Math.min(0 - CLEF_OVERHANG, Math.min(...steps) - 2)
